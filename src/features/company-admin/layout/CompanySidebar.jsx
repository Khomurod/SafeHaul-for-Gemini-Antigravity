...
export const CompanySidebar = () => {
  ...
  const { currentCompanyProfile, logout } = useData();
  ...
  const menuItems = [
    ...
    { type: 'element', element: <div className="my-2 border-t border-gray-700/50" /> },
      ...
      >
        {({ isActive }) => (
          <>
            ...
            {!isExpanded && (
              <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none z-50 whitespace-nowrap border border-gray-700">
                {item.label}
              </div>
            )}
          </>
  ...
  return (
    <div
      className={`
        h-screen bg-[#0F1115] border-r border-gray-800 flex flex-col transition-all duration-300
    ...
    >
        ...
        <div className="flex items-center gap-3 w-full overflow-hidden">
          ...
          {isExpanded && (
            <div className="flex flex-col min-w-0">
              ...
              <button onClick={() => navigate('/company/settings')} className="text-xs text-blue-400 hover:text-blue-300 text-left truncate">
                Switch Company
              </button>
            </div>
          )}
            ...
            return (
              <div key={idx} className="mb-2">
                <button
                  ...
                  className={`
                            w-full flex items-center gap-3 px-3 py-2 text-gray-400 hover:text-white hover:bg-gray-800/30 transition-colors
                            ${!isExpanded ? 'justify-center' : ''}
                            ${hasActiveChild ? 'text-blue-100' : ''}
                        `}
                >
                  <item.icon size={20} className={hasActiveChild ? 'text-blue-400' : ''} />
                  {isExpanded && (
                    <>
      ...
      <div className="p-3 border-t border-gray-800">
        <button
          ...
          className="w-full flex items-center justify-center p-2 text-gray-500 hover:text-white hover:bg-gray-800 rounded transition-colors"
        >
