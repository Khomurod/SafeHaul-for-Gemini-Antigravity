...
import {
    Activity, Play, Pause, RotateCcw, Terminal, Server, Database, HardDrive, ShieldCheck, Wrench, RefreshCw, DatabaseZap
} from 'lucide-react';
        ...
        <div className="space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                    ...
                    {/* BACKFILL PUBLIC PROFILES BUTTON */}
                    <button
                        onClick={runBackfillProfiles}
                        disabled={backfillStatus === 'running'}
                        className={`px-4 py-2 text-white font-bold rounded-lg shadow-md transition-all flex items-center gap-2
                    ${backfillStatus === 'running' ? 'bg-gray-400 cursor-not-allowed' :
                                backfillStatus === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-purple-600 hover:bg-purple-700'}`}
                        title="Force-sync all company data to public_profiles (fixes broken driver app links)"
                    >
