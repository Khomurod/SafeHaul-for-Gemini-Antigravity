...
import { Building, FileText, Edit2, Trash2, Search, ChevronLeft, ChevronRight, Loader2, Crown, Shield, MessageSquare, Phone, Database, ToggleLeft, ToggleRight, CheckCircle, XCircle } from 'lucide-react';
            ...
            <div className="flex-1 overflow-auto min-h-0 bg-gray-50">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                        {listLoading ? (
                            <tr><td colSpan="5" className="p-10 text-center text-gray-500"><Loader2 className="animate-spin mx-auto mb-2" />Loading companies...</td></tr>
                                        ...
                                        <button
                                            onClick={async (e) => {
                                                ...
                                                try {
                                                    ...
                                                    setCompanySearch(prev => prev + ' ');
                                                    setTimeout(() => setCompanySearch(prev => prev.trim()), 10);
                                                } catch (err) {
                                                    console.error('Failed to toggle status:', err);
                                                }
                                            }}
                                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${company.isActive !== false
                                                ? 'bg-green-100 text-green-700 border border-green-300 hover:bg-green-200'
                                                : 'bg-red-100 text-red-700 border border-red-300 hover:bg-red-200'
                                                }`}
                                            title={company.isActive !== false ? 'Click to deactivate' : 'Click to activate'}
                                        >
