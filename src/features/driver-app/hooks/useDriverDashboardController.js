...
// IMPORT THE NEW FUNCTIONS FROM THE SERVICE
import { 
    fetchDriverProfile, 
    fetchMyApplications, 
...
export function useDriverDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      // Load all data in parallel using the centralized service
      try {
          ...
          setApplications(userApps); // Now contains unique IDs and General Leads
          setSavedJobsData(savedJobs);

          // If profile exists and has a type, fetch jobs
  ...
  const refreshData = async () => {
      if(currentUser) {
          const apps = await fetchMyApplications(currentUser.email);
