...
/**
 * Smart Name Resolver for Leads.
 * Priorities: 
 * 1. Full Name (if valid)
 * 2. First + Last Name (if valid)
 * 3. Email Username
 * 4. Phone Number
 * 5. "SafeHaul Driver"
 */
export function getDisplayLeadName(lead) {
    if (!lead) return "SafeHaul Driver";

    // Helper to check if a string is "garbage"
    const isValid = (str) => {
        if (!str) return false;
    ...
    // 1. Try Full Name
    if (isValid(lead.fullName)) return lead.fullName;

    // 2. Try constructed name
    const f = lead.firstName;
    const l = lead.lastName;

    // Check if parts are valid individually
    const fValid = isValid(f);
    const lValid = isValid(l);

    if (fValid || lValid) {
        const cleanF = fValid ? f : '';
        const cleanL = lValid ? l : '';
        return `${cleanF} ${cleanL}`.trim();
    }

    // 3. Fallback to Email (Username)
    if (isValid(lead.email) && lead.email.includes('@')) {
        return lead.email.split('@')[0];
    }

    // 4. Fallback to Phone
    if (isValid(lead.phone)) {
        return formatPhoneNumber(lead.phone);
    }

    // 5. Final Fallback
    return "SafeHaul Driver";
}