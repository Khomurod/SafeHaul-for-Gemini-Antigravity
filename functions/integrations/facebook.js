...
exports.facebookWebhookV1 = functions.https.onRequest(async (req, res) => {
    ...
    // SH-006: Fail-closed when required secrets are not configured.
    // A missing secret must never result in an open webhook.
    if (!APP_SECRET || !VERIFY_TOKEN_VALUE) {
        console.error('[FacebookWebhook] FACEBOOK_APP_SECRET_VALUE or FACEBOOK_VERIFY_TOKEN_VALUE not configured. Rejecting request.');
        return res.sendStatus(500);
    }
    ...
    // B. Security (Signature Check for POST)
    // SH-006: Always enforce HMAC signature — APP_SECRET is guaranteed non-empty above.
    if (req.method === 'POST') {
        const signature = req.headers['x-hub-signature'];
        if (!signature) {
