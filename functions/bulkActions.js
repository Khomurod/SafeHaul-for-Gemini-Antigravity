...
exports.retryFailedAttempts = onCall(async (request) => {
  ...
  try {
    ...
    // Always fetch to get sourceType
...
exports.processBulkBatch = onRequest({
  ...
  try {
    ...
    const { targetIds, currentPointer, config, leadSourceType } = session;
    ...
    const companyName = config.companyName || "SafeHaul";
