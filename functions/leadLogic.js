...
async function processCompanyCleanup(companyId, now, forceRotate) {
  ...
  for (const docSnap of currentLeadsSnap.docs) {
    ...
    if (batchSize >= 400) {
      await batch.commit(); batch = db.batch(); batchSize = 0;
    }
  }
...
async function confirmDriverInterest(data) {
  ...
  const companyId = companyQuery.empty ? companyIdOrSlug : companyQuery.docs[0].id;
...
async function runCleanup() {
  ...
  for (const doc of leadsSnap.docs) {
    ...
    if (bSize >= 400) {
      await batch.commit(); batch = db.batch(); bSize = 0;
    }
  }
  ...
  for (const doc of companiesSnap.docs) {
    ...
    if (cSize >= 400) {
      await cBatch.commit(); cBatch = db.batch(); cSize = 0;
    }
  }
  ...
  for (const userDoc of usersSnap.docs) {
    try {
      ...
      if (e.code === "auth/user-not-found") {
        await userDoc.ref.delete(); deletedUsers++;
      }
    }
  }
